...
class LoginAccountant extends StatefulWidget {
  final String roles;
  const LoginAccountant({Key? key, required this.roles}) : super(key: key);

  @override
  _LoginAccountantState createState() => _LoginAccountantState();
}

class _LoginAccountantState extends State<LoginAccountant> {
  GlobalKey<FormState> _formKey = GlobalKey<FormState>();
  final GlobalKey<ScaffoldState> _scaffoldKey = GlobalKey<ScaffoldState>();
  String? email, password;
  var accountantToken, shipmentToken;
  ...
  doAccountantLOgin() async {
    if (this._formKey.currentState!.validate()) {
      ...
      var response = await Providers().loginAccountant(loginData);
      log(jsonEncode(response));
      if (response.status == true) {
        accountantToken = response.data[0].token;
        print("Accountanat Token $accountantToken");
        // print("Shipment Id $shipmentId");
        SharedPreferences prefs = await SharedPreferences.getInstance();
        prefs.setString('email', "$email");
        // prefs.setBool('isLoginShipment', true);
        // // prefs.setString('auth_token', shipmentToken);
        prefs.setString('Shipemnt_auth_token', accountantToken);
