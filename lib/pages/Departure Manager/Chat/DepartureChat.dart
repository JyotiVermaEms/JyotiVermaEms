...
class ChatDepartureScreen extends StatefulWidget {
  var data;
  ChatDepartureScreen(this.data, {Key? key}) : super(key: key);

  @override
  _ChatDepartureScreenState createState() => _ChatDepartureScreenState();
}
...
class _ChatDepartureScreenState extends State<ChatDepartureScreen> {
  var data = {};
  @override
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Responsive(
        mobile: DepartureChatScreen(widget.data),
        tablet: DepartureChatScreen(widget.data),
        desktop: Row(
          children: [
            ...
            Expanded(
              ...
              child: DepartureChatScreen(widget.data),
            ),
            // Expanded(
...
class DepartureChatScreen extends StatefulWidget {
  const DepartureChatScreen(data, {Key? key}) : super(key: key);

  @override
  _DepartureChatScreenState createState() => _DepartureChatScreenState();
}

class _DepartureChatScreenState extends State<DepartureChatScreen> {
  ...
  searchList(name) async {
    ...
    var token = prefs.get("depature_token");
  ...
  void getShipProfileDetails() async {
    ...
    var userDetails = await Providers().getDepatureProfile();
                    ...
                    child: TextFormField(
                      ...
                      decoration: InputDecoration(
                          ...
                          prefixIcon: IconButton(
                            onPressed: () {
                              if (searchControllerClient!.text.length > 2) {
                                searchList(searchControllerClient!.text);
                              }
                            },
                            icon: Icon(Icons.search, color: Colors.grey),
                          ),
                          suffixIcon: searchControllerClient!.text.isNotEmpty
                              ? IconButton(
                                  onPressed: () {
