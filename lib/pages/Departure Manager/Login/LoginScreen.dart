...
class DepLoginScreen extends StatefulWidget {
  final String roles;

  const DepLoginScreen({Key? key, required this.roles}) : super(key: key);

  @override
  _DepLoginScreenState createState() => _DepLoginScreenState();
}

class _DepLoginScreenState extends State<DepLoginScreen> {
  ...
  var depatureManagerToken;
  doDepatureManagerLOgin() async {
    if (this._formKey.currentState!.validate()) {
      ...
      var response = await Providers().loginDepatureManager(loginData);
      log(jsonEncode(response));
      if (response.status == true) {
        depatureManagerToken = response.data[0].token;
        print("depature Token $depatureManagerToken");
        // print("Shipment Id $shipmentId");
        SharedPreferences prefs = await SharedPreferences.getInstance();
        prefs.setString('email', "$email");
        prefs.setBool('isLoginShipment', true);
        // // prefs.setString('auth_token', shipmentToken);
        prefs.setString('depature_token', depatureManagerToken);
                      ...
                      Column(
                        ...
                        children: [
                          Container(
                              ...
                              child: Text("Sign in as Depature Manager",
                                  style: TextStyle(
                                      fontFamily: 'Roboto',
                                      color: Colors.white,
                                      fontSize: 28,
