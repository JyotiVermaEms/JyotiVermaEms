...
class DepartureProfile extends StatefulWidget {
  const DepartureProfile({Key? key}) : super(key: key);

  @override
  _DepartureProfileState createState() => _DepartureProfileState();
}

class _DepartureProfileState extends State<DepartureProfile> {
  ...
  Future updateProfileApi() async {
    ...
    var response = await Providers().updateDepatureProfile(udpateData);
    if (response.status == true) {
      setState(() {
        name = response.data[0].name;
        print("Name $name");
